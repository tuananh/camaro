...
function writeBytesToSab(channel, bytes) {
	if (bytes.byteLength > channel.xml.byteLength) {
		throw new RangeError(`XML payload (${bytes.byteLength} bytes) exceeds CAMARO_SAB_XML_BYTES (${channel.xml.byteLength})`);
	}
...
function writeStringXmlToSab(channel, xml) {
	...
	if (worstCase > channel.xml.byteLength) {
		throw new RangeError(`XML payload (> ${worstCase} bytes) exceeds CAMARO_SAB_XML_BYTES (${channel.xml.byteLength})`);
	}
...
function writeResultToSab(channel, payload) {
	...
	if (textEncoder) {
		...
		if (read !== str.length) {
			throw new RangeError(`Result exceeds CAMARO_SAB_RESULT_BYTES (${channel.result.byteLength})`);
		}
		...
		if (encoded.byteLength > channel.result.byteLength) {
			throw new RangeError(`Result (${encoded.byteLength} bytes) exceeds CAMARO_SAB_RESULT_BYTES (${channel.result.byteLength})`);
		}
...
function readResultFromSab(channel) {
	...
	const str = textDecoder ? textDecoder.decode(slice) : Buffer.from(slice).toString('utf8');
