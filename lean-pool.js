...
class LeanWorker {
	constructor(filename, idleTimeout, onRecycleXml, useSab) {
		...
		this.useSab = useSab;
		this.useSabWait = useSab && typeof Atomics.waitAsync === 'function' && typeof Atomics.notify === 'function';
	...
	ensureWorker() {
		...
		const workerOpts = this.sabChannel != null ? { workerData: { sab: this.sabChannel } } : undefined;
	...
	settleSabResult(id) {
		if (!this.sabChannel || Atomics.load(this.sabChannel.control, CTRL.STATE) !== STATE.DONE) {
			return;
		}
		const p = this.pending.get(id);
		this.pending.delete(id);
...
class LeanPool {
	constructor(filename, opts = {}) {
		...
		this.maxThreads = maxThreads || (envSize > 0 ? envSize : defaultMaxThreads());
		this.workers = [];
		this.queue = [];
		for (let i = 0; i < Math.min(DEFAULT_MIN_THREADS, this.maxThreads); i++) {
			const worker = new LeanWorker(this.filename, this.idleTimeout, this.onRecycleXml, this.useSab);
			worker.ensureWorker();
	...
	drain() {
		while (this.queue.length > 0) {
			...
			if (!worker && this.workers.length < this.maxThreads) {
				worker = new LeanWorker(this.filename, this.idleTimeout, this.onRecycleXml, this.useSab);
				this.workers.push(worker);
			}
			if (!worker) break;
